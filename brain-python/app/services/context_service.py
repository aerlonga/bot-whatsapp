...
# Carrega modelo de embeddings multimodal para criar vetor a partir dos resumos
...
async def _get_conn():
    ...
    # Registra o tipo vector no asyncpg se ele já estiver criado no banco
    try:
        await conn.execute("CREATE EXTENSION IF NOT EXISTS vector;")
        import asyncpg.types
        # asyncpg lida com arrays naturalmente, mas é uma boa prática
    except Exception as e:
        logger.warning(f"Erro ao tentar criar extensão vector: {e}")
    return conn

async def _get_or_create_contato(conn, telegram_user_id: str) -> int:
